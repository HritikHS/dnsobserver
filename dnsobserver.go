...
func handleInteraction(w dns.ResponseWriter, r *dns.Msg) {
	...
	if dns.IsSubDomain(conf.Domain+".", q1.Name) && q1.Name != "ns1."+conf.Domain+"." && q1.Name != "ns2."+conf.Domain+"." {
		...
		if conf.SlackWebhook != "" {
			if !strings.EqualFold("ns1.azoq.xyz.", q1.Name) && !strings.EqualFold("azoq.xyz.", q1.Name) && !strings.EqualFold("ns2.azoq.xyz.", q1.Name) && !strings.EqualFold("www.azoq.xyz.", q1.Name) && !strings.EqualFold("_dmarc.azoq.xyz.", q1.Name){
				sendSlack(message)
			}
		} else {
			fmt.Println(message)
